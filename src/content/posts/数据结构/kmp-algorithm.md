...
KMP（Knuth-Morris-Pratt）算法是 [串](./string-basics.md) 的模式匹配优化算法。其核心思想是：**利用已匹配信息确定模式串的滑动位置，主串指针永不回退**。

[BF算法（朴素模式匹配）](./sting-basics.md) 每次失配都要将主串指针回退到起点的下一位，大量比较被浪费。KMP 的洞察是：失配时，模式串中已匹配的部分包含了足够的信息来决定下一步从哪里继续比较。通过预先计算的 **next 数组**，模式串可以跳过那些不可能匹配的位置，而主串指针始终向前移动。
