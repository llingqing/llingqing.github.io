...
## 静态路由+TELNET
![image.png](https://tu.lingluoa.dpdns.org/file/obsidian/1783317873161_image.png)
`R1`配置
```shell
...
![image-2.png](https://tu.lingluoa.dpdns.org/file/obsidian/1783317899844_image-2.png)
`R1`配置
```shell
R1(config)#interface g0/0
...
![image-1.png](https://tu.lingluoa.dpdns.org/file/obsidian/1783317919722_image-1.png)
`sw3`配置
```shell
...
```
然后需要在server1上设置开启dns并添加记录，名称为inside.com
地址为20.0.0.2，然后添加
### 验证
...
给两个pc配上对应的ip后能ping通即可
## 标准的ACL
![image-3.png](https://tu.lingluoa.dpdns.org/file/obsidian/1783317927407_image-3.png)
`sw3`配置
```shell
Sw(config)#interface f0/2
Sw(config-if)#switchport access vlan 10
...
```
### 验证
`pc1`ping通`server1` `pc2`不可达`server1`
`show access-lists`
## 利用动态NAPT实现局域网访问互联网
![image-5.png](https://tu.lingluoa.dpdns.org/file/obsidian/1783317943641_image-5.png)
动态NAPT
`R1`配置
```shell
R1(config)#interface g0/1
...
```
### 验证
`pc0`ping通`server0`并能访问http://100.0.0.1
## 利用NAT实现外网主机访问内网服务器
![image-4.png](https://tu.lingluoa.dpdns.org/file/obsidian/1783317955872_image-4.png)
静态NAPT(`xp1`访问`serve1`http://200.1.8.9)
`R1`配置
```shell
R1(config)#interface g0/1
R1(config-if)#ip address 192.168.1.1 255.255.255.0
...
`xp1`成功访问`serve1`http://200.1.8.9